/**
 * Ops request triage core logic: Slack @opsteam mentions -> Asana "Ops Requests" board -> Slack updates.
 *
 ...
 * Stage 3: when a task is completed (ticked, or moved to the Done column), add ✅ to the
 *          original message and remove the bot's 👀.
 *
 ...
 *
 * Runs anywhere fetch exists: Cloudflare Workers (src/worker.js) and Node (src/sync.js).
 */
...
export async function runSync({ slackToken, asanaToken, channelIds }) {
  ...
  async function stageIntake(botUserId) {
    ...
    for (const channel of channels) {
      ...
      for (const msg of messages) {
        ...
        // (people may tag the bot by mistake since it's named "Ops Bot").
  ...
  return { created, announced, completed };
}
